...
def test(opt):
    ...
    for patient in patientlist:
        ...
        gradient_map = -1 * np.ones((image_data.shape[2], image_data.shape[3], image_data.shape[0]))
        laplacian_map = -1 * np.ones((image_data.shape[2], image_data.shape[3], image_data.shape[0]))
        ...
        for i in range(bbox[0][0],bbox[0][1]):
            ...
            aggregated_grads = torch.zeros([1] + list(data.shape[1:]), device=device)
            aggregated_laplacian = torch.zeros([1] + list(data.shape[1:]), device=device)
            ...
            for x in steps[0]:
                ...
                for y in steps[1]:
                    ...
                    gt_to_test = gt[None, :, lb_x:ub_x, lb_y:ub_y]
                    fake_img, coords = model(data_to_test, False, mode='inference_grad')
                    loss = criterionL1(fake_img,gt_to_test)
                    img_grad = gradient(fake_img,coords)
                    img_grad = img_grad.norm(dim=1).detach()
                    img_lapla = laplace(fake_img, coords).detach()
                    probs = fake_img.squeeze(0)
                    aggregated_results[:,lb_x:ub_x,lb_y:ub_y] += probs
                    aggregated_grads[:,lb_x:ub_x,lb_y:ub_y] += img_grad.squeeze(0)
                    aggregated_laplacian[:,lb_x:ub_x,lb_y:ub_y] +=img_lapla.squeeze(0)
                    aggregated_nb_of_predictions[:,lb_x:ub_x,lb_y:ub_y] += add_for_nb_of_preds
            slicer_trans = tuple(
                [slice(0, aggregated_results.shape[0])] + slicer[1:])

            aggregated_results = aggregated_results[slicer_trans]
            aggregated_grads = aggregated_grads[slicer_trans]
            aggregated_laplacian = aggregated_laplacian[slicer_trans]
            ...
            grad_slice = aggregated_grads / aggregated_nb_of_predictions
            laplacian_slice = aggregated_laplacian / aggregated_nb_of_predictions
            #prediction[:,:,i] = probs.cpu().numpy()
            prediction[bbox[1][0]:bbox[1][1],bbox[2][0]:bbox[2][1],i] = pred_slice.cpu().detach().numpy()
            gradient_map[bbox[1][0]:bbox[1][1],bbox[2][0]:bbox[2][1],i] = grad_slice.cpu().detach().numpy()
            laplacian_map[bbox[1][0]:bbox[1][1],bbox[2][0]:bbox[2][1],i] = laplacian_slice.cpu().detach().numpy()
        ...
        save_name = os.path.join(save_dir, patient + "_grad.nii.gz")
        nib.save(grad_to_save,save_name)
        
        laplacian_to_save = nib.Nifti1Image(laplacian_map, img_affine)
        save_name = os.path.join(save_dir, patient + "_laplacian.nii.gz")
        nib.save(laplacian_to_save,save_name)
