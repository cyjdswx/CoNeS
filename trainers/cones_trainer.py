...
from models.cones_model import ConesModel
...
class ConesTrainer():
    def __init__(self, opt, img_size):
        ...
        if opt.L1_loss:
            ...
            self.rec_loss = 'MSE'
        self.model = ConesModel(opt,img_size, device)
        self.generated = None
    ...
    def run_generator_one_step(self, data, use_gan) -> None:
        ...
        g_losses, generated = self.model(data, use_gan, mode='generator')
        print(g_losses)
    ...
    def run_discriminator_one_step(self, data) -> None:
        if self.optimizer_D is not None:
            ...
            d_losses = self.model(data, True, mode='discriminator')
    ...
    def run_evalutation_during_training(self, dataloader):
        ...
        with torch.no_grad():
            for i, batch in enumerate(dataloader):
                g_losses, generated = self.model(batch, self.opt.use_gan, mode='generator')
