...
class Visualizer():
    ...
    def display_current_seg(self,visuals,epoch,step):
        for label, image_numpy in visuals.items():
            ...
            if label == 'input':
                ...
                mask = tmp.squeeze().argmax(dim=0)
                save_image(mask.float(), img_path, normalize=True, range=(0,3))
            elif label == 'groundtruth':
                save_image(tmp.squeeze(), img_path, normalize=True, range=(0,3))
            ...
            for label, image_numpy in visuals.items():
                if isinstance(image_numpy, list):
                    ...
                    if label == 'groundtruth':
                        save_image(tmp.float().squeeze(), img_path, normalize=True, range=(0,3))
                    else:
                        save_image(tmp.float().squeeze(), img_path, normalize=True, range=(-1,1))
    ...
    def plot_current_errors(self, errors, step):
        if self.tf_log:
            for tag, value in errors.items():
                ...
                #print(tag,value.item(),step)

    # errors: same format as |errors| of plotCurrentErrors
    def print_current_errors(self, epoch, errors, t):
